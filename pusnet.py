...
parser.add_argument('--sr', default=0.3, type=float, help='sparse ratio')  
...
model_save_path = os.path.join(c.model_dir, 'pusnet', 'sr='+str(args.sr))
...
mkdirs('results')
logger_name = 'pusnet_trained_ON_' + c.data_name_train
...
# sparse_mask = generate_sparse_mask(model_hiding_seed, sparse_ratio=0.9, sc=args.mt)
...
train_loader, test_loader = load_dataset(train_data_dir, test_data_dir, c.pusnet_batch_size_train, c.pusnet_batch_size_test, c.pusnet_sigma)
...
if c.mode == 'test':
    ...
    with torch.no_grad():
        ...
        for idx, (data, noised_data) in enumerate(stream):
            ...
            if c.save_processed_img == True:
                ...
                for cur_dir in super_dirs:
                    mkdirs(os.path.join(c.IMAGE_PATH, 'pusnet', c.data_name_test, cur_dir))    
                image_name = '%.4d.' % idx + c.suffix
                save_image(cover, os.path.join(c.IMAGE_PATH, 'pusnet', c.data_name_test, super_dirs[0], image_name))
                save_image(secret, os.path.join(c.IMAGE_PATH, 'pusnet', c.data_name_test, super_dirs[1], image_name))
                save_image(stego, os.path.join(c.IMAGE_PATH, 'pusnet', c.data_name_test, super_dirs[2], image_name))
                save_image(secret_rev, os.path.join(c.IMAGE_PATH, 'pusnet', c.data_name_test, super_dirs[3], image_name))
                save_image(cover_resi, os.path.join(c.IMAGE_PATH, 'pusnet', c.data_name_test, super_dirs[4], image_name))
                save_image(secret_resi, os.path.join(c.IMAGE_PATH, 'pusnet', c.data_name_test, super_dirs[5], image_name))
                save_image(noised, os.path.join(c.IMAGE_PATH, 'pusnet', c.data_name_test, super_dirs[6], image_name))
                save_image(denoised, os.path.join(c.IMAGE_PATH, 'pusnet', c.data_name_test, super_dirs[7], image_name))
        ...
        logger.info('testing, stego_avg_psnr: {:.2f}, secref_avg_psnr: {:.2f}, noise_avg_psnr: {:.2f}, denoise_avg_psnr: {:.2f}'.format(np.mean(S_psnr), np.mean(R_psnr), np.mean(N_psnr), np.mean(DN_psnr)))
        logger.info('testing, stego_avg_ssim: {:.4f}, secref_avg_ssim: {:.4f}, noise_avg_ssim: {:.4f}, denoise_avg_ssim: {:.4f}'.format(np.mean(S_ssim), np.mean(R_ssim), np.mean(N_ssim), np.mean(DN_ssim)))
        logger.info('testing, stego_avg_mae: {:.2f}, secref_avg_mae: {:.2f}, noise_avg_mae: {:.2f}, denoise_avg_mae: {:.2f}'.format(np.mean(S_mae), np.mean(R_mae), np.mean(N_mae), np.mean(DN_mae)))
        logger.info('testing, stego_avg_rmse: {:.2f}, secref_avg_rmse: {:.2f}, noise_avg_rmse: {:.2f}, denoise_avg_rmse: {:.2f}'.format(np.mean(S_rmse), np.mean(R_rmse), np.mean(N_rmse), np.mean(DN_rmse)))
    ...
    for epoch in range(c.epochs):
        ...
        if epoch % c.test_freq == 0:
            with torch.no_grad():
                ...
                logger.info('epoch: {}, testing, stego_avg_psnr: {:.2f}, secref_avg_psnr: {:.2f}, noise_avg_psnr: {:.2f}, denoise_avg_psnr: {:.2f}'.format(epoch, np.mean(S_psnr), np.mean(R_psnr), np.mean(N_psnr), np.mean(DN_psnr)))
